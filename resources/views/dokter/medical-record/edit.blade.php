        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <form action="{{ route('medicalrecord.update', $record->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                            ...
                            <div>
                                ...
                                <input type="text" name="penyakit" id="penyakit" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" value="{{ old('penyakit', $record->penyakit) }}" required>
                            </div>

                            <!-- Tindakan Medis -->
                            <div>
                                <label for="tindakan" class="block text-sm font-medium text-gray-700">Tindakan Medis</label>
                                <textarea name="tindakan" id="tindakan" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" required>{{ old('tindakan', $record->tindakan) }}</textarea>
                            </div>
                                ...
                                @foreach ($record->obats as $obat)
                                    <div class="obat-item flex space-x-4 mt-2">
                                        <select name="obat_id[]" class="obat-select mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" required>
                                            ...
                                            @foreach ($obats as $availableObat)
                                                <option value="{{ $availableObat->id }}" {{ $availableObat->id == $obat->id ? 'selected' : '' }}>{{ $availableObat->nama_obat }} (Stok: {{ $availableObat->stok }})</option>
                                            @endforeach
                                        </select>
                                        <input type="number" name="jumlah_obat[]" class="jumlah-obat mt-1 block w-24 rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" min="1" value="{{ old('jumlah_obat.' . $obat->id, $obat->pivot->jumlah) }}" required>
